...
HandleRFBServerMessage(rfbClient* client)
{
  ...
  case rfbFramebufferUpdate:
  {
    ...
    /*
	\note [jarkko] WHAT THE FUCK are the authors smoking? Who the fuck
	               decided that it would be a great idea to make library
				   for pull based protocol to silently always behave like
				   a pub-sub. Not just regular pub-sub but a shitty one
				   without any control given to user. Like ability to
				   unsubscribe.
				   Why this is shit:
						- RFB cannot be put on hold while app is not visible
						- If user writes his app based on the documentation,
						  it WILL lead into requesting frames faster than app
						  can consume which WILL cause huge TCP buffering
						  leading to shitty latencies.
